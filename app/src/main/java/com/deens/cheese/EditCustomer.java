...
public class EditCustomer extends AppCompatActivity implements AdapterView.OnItemSelectedListener, OnMapReadyCallback,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lm = (LocationManager) EditCustomer.this.getSystemService(Context.LOCATION_SERVICE);
        ...
        mapFragment.getMapAsync(EditCustomer.this);
        ...
        forward.setOnClickListener(view -> {
            hideSoftKeyboard(EditCustomer.this);
        ...
        back.setOnClickListener(view -> {
            EditCustomer.super.onBackPressed();
            overridePendingTransition(R.anim.slide_in_left,
                    R.anim.slide_out_right);
        });

        registrationDate.setOnClickListener(view -> {
            new DatePickerDialog(EditCustomer.this, date, myCalendar
                    .get(Calendar.YEAR), myCalendar.get(Calendar.MONTH),
                    myCalendar.get(Calendar.DAY_OF_MONTH)).show();
        });

        if (isNetworkAvailable()) {
            EditCustomer.GetAllowedUserTyps task = new EditCustomer.GetAllowedUserTyps();
            task.execute();
        } else {
                    ...
                    if (soList.size() <= 0){
                        ...
                        if (loggedInUserType.equals("6") || loggedInUserType.equals("7")){
                            try {
                                ...
                                if (isNetworkAvailable()) {
                                    EditCustomerDetail task = new EditCustomerDetail();
                                    task.execute();
                                } else {
...
//                        }
                        ...
                        if (isNetworkAvailable()) {
                            EditCustomer.GetEmployeesList task = new EditCustomer.GetEmployeesList();
                            task.execute();
                        } else {
                            SnackAlert.error(parentView, "Internet not available!");
                        }
        ...
        protected void onPostExecute(Void a) {
            if (!isFinishing()){
                ...
                if (!message.equals("")) {
                    ...
                    if (isNetworkAvailable()) {
                        EditCustomer.GetDataFromServer task = new EditCustomer.GetDataFromServer();
                        task.execute();
                    } else {
                        SnackAlert.error(parentView, "Internet not available!");
                    }
    ...
    private class EditCustomerDetail extends AsyncTask<Void, Void, Void> {

        String methodName = "Customer/UpdateCustomer?";
    ...
    private void loadDaysSpinner() {
        ...
        ArrayAdapter adapter = new ArrayAdapter(EditCustomer.this, R.layout.spinner_item, days);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        daySpinner.setAdapter(adapter);
    }

    // We will load selected customer data here
    private void loadMiscSpinners() {
        ...
        // Load Selected Customer Data from selected object //
        ...
        runOnUiThread(() -> {
            ...
            if (gmIds.contains(selectedCustomer.getGM())) {
                gmSpinner.setSelection(gmIds.indexOf(selectedCustomer.getGM()));
            }
            ...
            new Handler().postDelayed(() -> {
                ...
                if (stIds.contains(selectedCustomer.getST())) {
                    stSpinner.setSelection(stIds.indexOf(selectedCustomer.getST()));
                }
            }, 1000);
            ...
            new Handler().postDelayed(() -> {
                ...
                if (dmIds.contains(selectedCustomer.getDM())) {
                    dmSpinner.setSelection(dmIds.indexOf(selectedCustomer.getDM()));
                }
            }, 1500);
            ...
            new Handler().postDelayed(() -> {
                ...
                if (sdmIds.contains(selectedCustomer.getSDM())) {
                    sdmSpinner.setSelection(sdmIds.indexOf(selectedCustomer.getSDM()));
                }
            }, 2000);
            ...
            new Handler().postDelayed(() -> {
                ...
                if (soIds.contains(selectedCustomer.getSO_ID())) {
                    soSpinner.setSelection(soIds.indexOf(selectedCustomer.getSO_ID()));
                }
            }, 3000);

            latitude.setText(String.valueOf(selectedCustomer.getLat()));
            longitude.setText(String.valueOf(selectedCustomer.getLong()));
            customerLocation.setText(String.valueOf(selectedCustomer.getMapLocation()));
            customerName.setText(selectedCustomer.getCustomerName());
            citySpinner.setSelection(cityNames.indexOf(selectedCustomer.getCityName()));
            area.setText(selectedCustomer.getArea());
            address.setText(selectedCustomer.getAddress());
            contactNo.setText(selectedCustomer.getMobile());
            ntnNo.setText(selectedCustomer.getNTNNo());
            stnNo.setText(selectedCustomer.getSTNNo());
            isTax.setChecked(selectedCustomer.getIs_Tax_Register());
            contactNo2.setText(selectedCustomer.getMobile1().equals("null")? "-": selectedCustomer.getMobile1());
            registrationDate.setText(selectedCustomer.getRegistrationDate().replace("T00:00:00", ""));
            String[] days = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
            daySpinner.setSelection(Arrays.asList(days).indexOf(selectedCustomer.getVisit_Day()
                    .replace("Tuesdday", "Tuesday")));
        });
    }
        ...
        switch (index) {
            case 2:
                ...
                if (!isSelectable) {
                    ...
                    populateSpinnerWithArray(gmSpinner, new String[]{selectedCustomer.getGMName()});
                } else {
            ...
            case 3:
                ...
                if (!isSelectable) {
                    ...
                    populateSpinnerWithArray(gmSpinner, new String[]{selectedCustomer.getSTName()});
                } else {
                    if (isOnCLickListener) {
            ...
            case 4:
                ...
                if (!isSelectable) {
                    ...
                    populateSpinnerWithArray(gmSpinner, new String[]{selectedCustomer.getDMName()});
                } else {
                    if (isOnCLickListener) {
            ...
            case 5:
                ...
                if (!isSelectable) {
                    ...
                    populateSpinnerWithArray(gmSpinner, new String[]{selectedCustomer.getSDMName()});
                } else {
                    if (isOnCLickListener) {
            ...
            case 6:
                ...
                if (!isSelectable) {
                    ...
                    populateSpinnerWithArray(gmSpinner, new String[]{selectedCustomer.getSO_Name()});
                } else {
                    if (isOnCLickListener) {
    ...
    private void populateSpinner(Spinner spinner, ArrayList<EmployeeClass> list) {
        ...
        ArrayAdapter adapter = new ArrayAdapter(EditCustomer.this, R.layout.spinner_item, names.toArray());
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinner.setAdapter(adapter);
    }

    private void populateSpinnerWithArray(Spinner spinner, String[] array) {
        ArrayAdapter adapter = new ArrayAdapter(EditCustomer.this, R.layout.spinner_item, array);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinner.setAdapter(adapter);
    }
    ...
    private void changeSwitchSelection(RadioButton selected, RadioButton one, RadioButton two) {
        EditCustomer.this.runOnUiThread(() -> {
    ...
    private void loadSpinner(Spinner spinner, ArrayList<String> list) {
        ArrayAdapter adapter = new ArrayAdapter(EditCustomer.this, R.layout.spinner_item, list.toArray());
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinner.setAdapter(adapter);
    }
    ...
    public void onConnected(@Nullable Bundle bundle) {
        lat = String.valueOf(selectedCustomer.getLat());
        lng = String.valueOf(selectedCustomer.getLong());
        loc = selectedCustomer.getMapLocation();
            ...
            switch (status.getStatusCode()) {
                ...
                case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                    ...
                    try {
                        ...
                        status.startResolutionForResult(EditCustomer.this, 1000);
                    } catch (IntentSender.SendIntentException e) {
                        // Ignore the error.
                    }
                    break;
...
//                            Toast.makeText(LocationSelect.this, "LOCATION API NOT CONNECTED YET", Toast.LENGTH_SHORT).show();
    ...
    private String returnLocation() {
        ...
        geocoder = new Geocoder(EditCustomer.this, Locale.getDefault());
        ...
        try {
            ...
            e.printStackTrace();
            Toast.makeText(EditCustomer.this, "WRONG MAP DATA", Toast.LENGTH_LONG).show();
        }
